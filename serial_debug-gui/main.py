...
co2 = (
    tk.StringVar()
)  # Taking the Humidity values and storing it in a string (humidity).
tk.Label(root, textvariable=co2, font=("Lato", 20, "bold")).grid(
    row=1, column=1, sticky="w"
)  # Displaying the humidity value on GUI.

tk.Label(root, text="CO2 average:", font=("Lato", 20, "bold")).grid(
...
co2_average = (
    tk.StringVar()
)  # Taking the Temperature values and storing it in a string (temperature).
tk.Label(root, textvariable=co2_average, font=("Lato", 20, "bold")).grid(
    row=2, column=1, sticky="w"
)  # Displaying the temperature value on GUI.

tk.Label(root, text="Temperature:", font=("Lato", 20, "bold")).grid(
    row=3, column=0, sticky="w"
)  # Displaying a label (Soil Moisture) in GUI.
temperature = (
    tk.StringVar()
)  # Taking the Soil Moistue values and storing it in a string (percentvalue).
...
threading.Thread(
    target=readSerialData, daemon=True
).start()  # Doing threading in arduino function.
